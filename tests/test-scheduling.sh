...
SPUR_BIN_DIR="${SPUR_BIN_DIR:-$REPO_DIR/spur-bin}"
...
job_state() { ctl_spur show job "$1" 2>/dev/null | { grep -oE 'JobState=[A-Z_]+' || true; } | head -1 | cut -d= -f2; }
node_state() { ctl_spur show node "$1" 2>/dev/null | { grep -oE 'State=[A-Za-z*+]+' || true; } | head -1 | cut -d= -f2; }
...
echo ran on \$(hostname)"
  ...
  local st
  st=$(wait_job_state "$jid" "PENDING" 15)
  if [ "$st" = "PENDING" ]; then
...
echo should-not-run"
  ...
  for i in $(seq 1 30); do
    st_b=$(job_state "$jid_b")
    reason=$(ctl_spur show job "$jid_b" 2>/dev/null | { grep -oE 'Reason=[A-Za-z_]+' || true; } | head -1 | cut -d= -f2)
    case "$st_b" in CANCELLED|FAILED) break ;; esac
    case "$reason" in Dependency|DependencyNeverSatisfied) break ;; esac
    sleep 1
  done
  st_b=$(job_state "$jid_b")

  if [ "$st_a" = "FAILED" ] && [ "$st_b" != "COMPLETED" ] && { [ "$reason" = "DependencyNeverSatisfied" ] || [ "$reason" = "Dependency" ]; }; then
    pass "Test 4: parent FAILED, child=$st_b Reason=$reason"
  else
    fail "Test 4: parent=$st_a child=$st_b reason=$reason (expected FAILED + Dependency/DependencyNeverSatisfied)"
  fi
}
...
echo should-never-run"
  ...
  if [ "$rc" -ne 0 ]; then
    ...
    if [ -n "$jid" ]; then
      local st
      st=$(wait_job_state "$jid" "PENDING" 10)
...
test_11_drain_all_resume_all() {
  ...
  for a in gpu-1 gpu-2 gpu-3; do
    wait_node_state "$a" "drain|DRAIN" 20 >/dev/null
  done
...
test_12_cluster_state_consistency() {
  ...
  for a in gpu-1 gpu-2 gpu-3; do
    local st
    st=$(wait_node_state "$a" "IDLE|idle" 15)
    case "$st" in
      IDLE|idle) log "  $a: $st" ;;
