...
- **[`skills/diagnose-job/`](skills/diagnose-job/SKILL.md)** — Claude Code skill that diagnoses why a Spur job won't start, failed, timed out, got OOM-killed, or is otherwise misbehaving. Covers all job states and 60+ pending reasons.
...
Building the `spur`/`spurctld`/`spurd`/`spurstepd` binaries still happens in [ROCm/spur](https://github.com/ROCm/spur) (`cargo build --release -p spur-cli -p spurctld -p spurd -p spur-stepd`). `spurstepd` is the per-job supervisor `spurd` spawns; it must be installed in the same directory as `spurd`, which the playbooks handle. Release tarballs also include `spur_mpi_pmix.so`; the Ansible playbooks install it on agents at `/usr/lib/spur/`. See `ansible/README.md` for the full clone-build-stage-deploy walkthrough. This repo only consumes those binaries (via `spur_binary_src`/`SPUR_BINARY_SRC`) or the upstream `install.sh`.
