...
- **The master `spur.conf` (the `-i` argument) is the single source of truth** — `spur_controller`/`spur_agent` push its content (read via the `inventory_file` magic var) to each node, they never render it from Ansible facts/vars. There's no `delegate_facts`-based cross-host fact propagation left in these roles; everything a task needs (controller addresses, wg addresses, node names) is already a plain hostvar the `spur_conf` inventory plugin set upfront.
