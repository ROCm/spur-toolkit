...
| `spur_force_drain_settle_secs` | `5` | `rolling_upgrade.yml`'s force/trust-stepd modes only: short cap on the drain-wait above (they don't need to wait out a real job, just let the drain state transition itself settle). |
