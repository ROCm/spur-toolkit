...
**Overlays** (displayed as **replacements** in `sinfo` — the base state is hidden):
- `plnd` — backfill scheduler reserved a future slot on this node (base: idle)
- `resv` — held by an admin reservation
...
sinfo -N --format="%20N %10P %6t %10e %10m %20G %30f"
# Reason field is not available via sinfo format; use scontrol for drain/down reasons:
...
# Note: ReqMem is accepted as a field name but always displays "?" (not yet wired)
...
# Note: plnd is an overlay on idle nodes (displayed as a replacement, not suffix)
sinfo -N --format="%20N %10P %6t %20G %30f"
```

**Diagnosis:** Show busy (alloc/mix) vs idle vs backfill-reserved (`plnd`) nodes.
...
### E. Partition problems — `PartitionInactive` / `PartitionNodeLimit` / `PartitionTimeLimit` / `PartitionConfig`

```bash
scontrol show partition <partition_name>
```

- `PartitionInactive`: partition is down/inactive — admin brings it up or user moves partitions.
- `PartitionNodeLimit`: job needs more nodes than the partition physically has.
- `PartitionTimeLimit`: `--time` exceeds partition `MaxTime`.
- `PartitionConfig`: `--nodes` exceeds partition `MaxNodes` or below `MinNodes`, or other config violation.
...
sinfo -N --format="%20N %10P %6t %10c %10m %20G %30f"
scontrol show node | grep -E "NodeName|State|Reason"
```
...
| `MaxSubmitJobsPerAccount` | Submit count per account exceeded |
...
Fix: cancel and resubmit without `--begin`, or admin adjusts the job's start constraints.
...
Array `%N` concurrency cap reached. Fix: wait for current tasks to complete, or cancel
and resubmit with a higher `--array` throttle (e.g. `--array=0-99%20`).
...
### M. Kubernetes reserved — `ReqNodeNotAvail, Reserved for Kubernetes cluster`

Nodes in managed k0s cluster are excluded. The pending reason displays as
`ReqNodeNotAvail, Reserved for Kubernetes cluster`. Use non-k0s nodes.

### N. Accounting / validation — `AccountingUnavailable` / `InvalidQOS`

- `AccountingUnavailable`: QOS or association limit caches not yet readable (e.g. after
  controller restart while association data loads). Check `systemctl status spurctld`.
- `InvalidQOS`: QOS doesn't exist in the QOS cache. Check `sacctmgr show qos`.

> Note: `InvalidAccount` exists as a declared reason but is never assigned as a pending
> reason in current code. Account issues surface as submit-time rejections instead.
...
Fix: cancel and resubmit with a later `--deadline`, or without `--deadline` entirely.
...
scontrol show node <nodelist> | grep -E "NodeName|State|Reason"
```
...
scontrol show node <failed_node> | grep -E "NodeName|State|Reason"
...
- **Admin drain during job**: `scontrol update NodeName=<n> State=drain` does NOT kill running jobs — they complete naturally while the node transitions from `Draining` to `Drain`. Only new jobs are blocked.
...
sinfo -N --nodes=<nodelist> --format="%20N %6t"
scontrol show node <nodelist> | grep -E "NodeName|State|Reason"
...
| `no default account resolved for user` | `require_association=true` and user has no default account | `sacctmgr modify user name=<name> set defaultaccount=<acct>` |
| `user has no associations` | User not in any account | `sacctmgr add user name=<name> account=<acct>` |
| `user is not a member of account` | Explicit `--account` not associated | `sacctmgr add user name=<name> account=<acct>` |

### QOS / partition ACL errors

| Error message | Cause | Fix |
|---------------|-------|-----|
| `QoS '<qos>' not permitted for user/account association` | QOS not authorized for user/account | `sacctmgr modify user <name> set qos+=<qos>` |
| `default QOS does not exist` | Configured default QOS missing | `sacctmgr add qos <name>` |
| `QoS '<qos>' not allowed on partition '<part>'` | Partition restricts QOS list | Use an allowed QOS or admin updates partition |
| `QoS '<qos>' denied on partition '<part>'` | Partition blocks this QOS | Use a different QOS |
| `account '<acct>' not allowed on partition '<part>'` | Partition restricts account list | Use allowed account or admin updates partition |
| `account '<acct>' denied on partition '<part>'` | Partition blocks this account | Use a different account |
...
| Error message | Cause | Fix |
|---------------|-------|-----|
| `only one GPU request form (...) may be set` | Multiple conflicting GPU request forms | Use only one of `--gpus`, `--gpus-per-node`, `--gpus-per-task`, or a gpu gres entry |

### Validation / hook errors

| Error message | Cause | Fix |
|---------------|-------|-----|
| `partition '<name>' not found` | Partition doesn't exist | `sinfo` to list valid partitions |
...
| `permission denied` | User not authorized for the operation (authz, not exec bit) | Check user/account associations and admin status |
| `job submit hook rejected` / `job_submit hook is not executable` | Job submit hook vetoed or misconfigured | Check hook scripts on controller |
...
sinfo -N --states=down,drain,drng,err --format="%20N %6t"
scontrol show node | grep -E "NodeName|State|Reason" | grep -B1 -A1 -iE "down|drain|err"
...
sinfo -N --states=unk --format="%20N %6t"
scontrol show node | grep -E "NodeName|State|Reason" | grep -B1 -A1 -i "unk"

# Suspended nodes — power-managed, unavailable
sinfo -N --states=susp --format="%20N %6t"
...
- **`plnd` overlay:** Nodes showing `plnd` in `sinfo` look idle but have a backfill
  shadow reservation for a future higher-priority job. They won't accept new work until
  that reservation is honored or the reserved job cancels. Note: the overlay displays as
  a replacement (bare `plnd`), not as a suffix like `idle+PLANNED`.
...
- **Accounting bootstrap:** After controller restart, jobs can pend on
  `AccountingUnavailable` while QOS/association limit caches load. `InvalidQOS`
  can also appear as a pending reason if the QOS cache is not yet populated.
...
- **TIMEOUT grace period:** When wall-time expires, Spur sends SIGTERM first, then
  SIGKILL after 30 seconds (hardcoded `GRACE_PERIOD_SECS`, not configurable). If the
  job traps SIGTERM and doesn't exit, it gets hard-killed. ExitCode will show signal 9,
  but state is TIMEOUT.
